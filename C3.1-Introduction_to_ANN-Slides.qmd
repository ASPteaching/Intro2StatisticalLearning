...
- $x$ is the input,
- $\hat{y}$ is the output,
- $\theta = \{W^{(l)}, b^{(l)}\}_{l=1}^L$ are the parameters.

The function $f$ is obtained by *composing linear and non-linear transformations*:
$$
...
> Most common in implementations
...
> Same model, more compact notation
:::
...
$$
WHich should provide a better intuition for this oss function.
