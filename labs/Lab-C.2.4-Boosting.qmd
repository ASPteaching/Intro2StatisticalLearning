...
Best results obtained above can be extracted to obtain an improved predictor.

Altrnatively a grid search for the best parameter combination can be performed.

```{r prepareOptimization, eval=FALSE}
...
ames_test_matrix <- xgb.DMatrix(
  data = as.matrix(ames_test[, !names(ames_test) %in% "Sale_Price"]),
  label = test_labels
)
...
An optimal predictor can now be obtained training a model with the optimal values.

```{r optimalPredictor, eval=FALSE}
set.seed(123)
ames.boost.optimized <- xgb.train(
  data = ames_train_matrix,
  params = list(
    eta = best_params$eta,
    max_depth = 6,
...
ames.boost.optimized.trainpred <- predict(ames.boost.optimized, newdata = ames_train_matrix)
ames.boost.optimized.pred <- predict(ames.boost.optimized, newdata = ames_test_matrix)


train_rmseboost_optimized <- sqrt(mean((ames.boost.optimized.trainpred - getinfo(ames_train_matrix, "label"))^2))
test_rmseboost_optimized <- sqrt(mean((ames.boost.optimized.pred - getinfo(ames_test_matrix, "label"))^2))
```

The comparison shows the improvement

```{r optimalPredictorError, eval=FALSE}

cat("\nComparación de RMSE:\n")
cat("Modelo original - Train RMSE:", round(train_rmseboost, 2), "| Test RMSE:", round(test_rmseboost, 2), "\n")
cat("Modelo optimizado - Train RMSE:", round(train_rmseboost_optimized, 2), "| Test RMSE:", round(test_rmseboost_optimized, 2
