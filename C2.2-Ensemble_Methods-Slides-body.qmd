...
```{r, fig.align='center', fig.cap='The bias-variance trade-off'}
knitr::include_graphics("images/1.2-Ensemble_Methods-Slides_insertimage_1.png")
```
...
[Ensemble methods cheatsheet](https://sites.google.com/view/datascience-cheat-sheets#h.t1jchxvxlgr2)
...
<!-- - **Random Patches** -->
<!--   - Random subsets of both observations and features used to build each tree. -->
...
- On average, each bootstrap sample leaves out about one third of the observations:
...
```{r, out.width="80%", fig.cap="RF Algorithm, from ch. 11 in [@Boehmke2020]"}
...
## So bagging or boosting? {.smaller}
...
## A sequential correction {.smaller}
...
- It proceeds **Ada**ptatively by sequentially training weak,learners.
...
## Why do learner weights appear? {.smaller}
...
If a learner has weighted error rate, $\epsilon_t$, 
its contribution to the final ensemble is determined by:
$$
...
knitr::include_graphics("images/Adaboost.jpeg")
```
...
knitr::include_graphics("images/Adaboost4FaceRecognition.png")
```
...
## Moving beyond AdaBoost {.smaller}
...
## A function optimization  {.smaller}

Gradient Boosting views learning as an optimization problem.

Instead of directly searching for a single complex predictor,
the model is *built incrementally* as:
...
At each iteration, the algorithm searches for the weak learner
that most reduces the current loss function.

The final model is constructed through a sequence
of small corrective updates.
...
::: {.font90}
- For squared error loss,  \small{$L(y,F(x))=(y-F(x))^2$ }
the negative gradient becomes:
\small{$-\partial{L}/\partial{F}=y-F(x)$}, which *corresponds exactly to the residuals*.

- Therefore, for least-squares regression, fitting the negative gradient, and fitting the residuals, are equivalent procedures.

- This explains why Gradient Boosting is often described
as "iteratively fitting residuals".
...
2. For each iteration $m=1,\dots,M$:
   ...
   c. Find the optimal step size:$\gamma_m$.
...
```{r, out.width="100%"}
knitr::include_graphics("images/gradientboosting.jpeg")
```
<small>
Source: [Ensemble Learning: Bagging & Boosting](https://resources.experfy.com/ai-ml/ensemble-learning-bagging-boosting/)
</small>
